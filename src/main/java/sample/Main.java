...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
         ...
         */
        nickLabel = new Label("Podaj nick użytkownika:");
         ...
         */
        downloadLabel = new Label("Co chcesz pobrać?");
                ...
                if (validateInputNickTextField() == true) {
                    if(validateInputPathDirectoryTextField() == true) {
                        ...
                        if (downloadPhotosButton.isSelected()) {
                            ...
                            System.setProperty("webdriver.chrome.driver", "C:\\Users\\Krolik\\IdeaProjects\\InstaScrapperGUI\\InstaScrapperGUI\\chromedriver.exe");
                            //System.setProperty("webdriver.chrome.driver", "TUTAJ SCIEZKA DO CHROME DRIVERA NA DYSKU");
                            //LINK DO CHROME DRIVER WINDOWS: https://chromedriver.storage.googleapis.com/2.39/chromedriver_win32.zip

                            WebDriver driver = new ChromeDriver();

                            driver.get("https://www.instagram.com/" + nickTextField.getText());

                            ImageScraper scraper = new ImageScraper(driver);

                            try {
                                scraper.renderAllPhotos();
                                scraper.imgScrap(selectedDirectory.getAbsolutePath());

                                alertMessage = new Alert(Alert.AlertType.INFORMATION);
                                alertMessage.setTitle("Zakończono");
                                alertMessage.setHeaderText(null);
                                alertMessage.setContentText("Pobieranie zakończone powodzeniem!");
